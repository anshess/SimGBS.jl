...
function digest(sequence, re::restrictionEnzyme)
    ...
    endSeq =
        [SubString(cutSeq[i], 1, cutPlace[i] - 1) * overhang[i] for i = 1:length(cutSeq)]
...
function digestGenome(
    ...
    for c = 1:numChr
        ...
        println(
            "CHROMOSOME $c: $numFrag GBS fragments are generated through virtual digestion using $(reName).",
        )
        ...
        if useChrLen == [] || size(useChrLen, 1) < numChr
            ...
            println(
                "CHROMOSOME $c: Subsetting chromosome is not required. Keeping all $numFrag available GBS fragments.",
            )
        elseif size(useChrLen, 1) == numChr || useChrLen[c] <= estLen
            ...
            println(
                "CHROMOSOME $c (up to $(useChrLen[c]) cM): $(length(keep)) out of $numFrag available GBS fragments are kept.",
            )
        elseif size(useChrLen, 1) == 1 # keep only fragments found within specified length (uniform across chromosomes)
            ...
            error(
                "CHROMOSOME $c: Wrong chromosome length(s) specified, please check your parameters!",
            )
        end
    ...
    if plotOutput == true
        histogram(
            ...
            title = "Size Distribution of Raw GBS Fragements",
            label = "",
            fmt = :png,
        )
        savefig("RawFragSize")
        histogram(
            fragGBS[:, 4],
            normalize = :probability,
            title = "Size Distribution of GBS Fragements (after Size-Selection)",
            label = "",
            fmt = :png,
        )
        savefig("GBSFragSize")
