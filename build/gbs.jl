...
function GBS(
    ...
    if onlyOutputGBS == true
        ...
        snpPos = [snpPos[c][snpGBS[c]] for c = 1:numChr]   # snpPosGBS = [snpPos[c][snpGBS[c] .- [0;length.(snpPos)][c]] for c in 1:numChr] # positions of SNPs captured by GBS fragments
    end
    ...
    matDepthFwd = [rand(Binomial(matHapDepth[i, j])) for i = 1:totalInd, j = 1:totalFrag]
    matDepthRevComp = matHapDepth - matDepthFwd
    patDepthFwd = [rand(Binomial(patHapDepth[i, j])) for i = 1:totalInd, j = 1:totalFrag]
            ...
            for f = 1:length(fragSelected)
                ...
                readGBS = collect(
                    Iterators.flatten([
                        [
                            rep(
                                [barcodes[i] .* matHapChrTemp[i]],
                                matDepthFwd[useInd[i], f],
                            )
                            rep(
                                [barcodes[i] .* matHapRevCompChr[i]],
                                matDepthRevComp[useInd[i], f],
                            )
                            rep([barcodes[i] .* patHapChr[i]], patDepthFwd[useInd[i], f])
                            rep(
                                [barcodes[i] .* patHapRevCompChr[i]],
                                patDepthRevComp[useInd[i], f],
                            )
                        ] for i = 1:numInd
                    ]),
                )
    ...
    snpDepth = transpose(
        reshape(
            vcat(
                [
                    rep(readDepth[i, :], collect(Iterators.flatten(hapSize))) for
                    i = 1:totalInd
                ]...,
            ),
            sum(hapSize),
