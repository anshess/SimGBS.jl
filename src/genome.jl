...
mutable struct restrictionEnzyme
    ...
    restrictionEnzyme(name, cutSeq, cutPlace, overhang) =
        new(name, cutSeq, cutPlace, overhang)
end
...
nucl = Dict(
    ...
    'H' => rand(['A', 'C', 'T']),
    'V' => rand(['A', 'C', 'G']),
); # randomly assign IUPAC codes for uncertain ones in the assembly
...
function digestGenome(
    genofile::String,
    re::Array{restrictionEnzyme,1},
    useChr::Array{Int64},
    useChrLen::Array{Float64},
    lowerThresh::Int64,
    upperThresh::Int64,
    winSize::Int64,
    plotOutput::Bool,
    ...
    for c = 1:numChr
        ...
        println(
            "CHROMOSOME $(useChr[c]): $numFrag GBS fragments are generated through virtual digestion using $(reName).",
        )
        ...
        println(
            "CHROMOSOME $(useChr[c]): Estimated chromosome length equals to $chrLenEst Mb.",
        )
        ...
        if useChrLen == [] || size(useChrLen, 1) < numChr
            ...
            println(
                "CHROMOSOME $(useChr[c]): Subsetting chromosome is not required. Keeping all $numFrag available GBS fragments.",
            )
        elseif size(useChrLen, 1) == numChr || useChrLen[c] <= estLen
            ...
            println(
                "CHROMOSOME $(useChr[c]) (up to $(useChrLen[c]) cM): $(length(keep)) out of $numFrag available GBS fragments are kept.",
            )
        elseif size(useChrLen, 1) == 1 # keep only fragments found within specified length (uniform across chromosomes)
            ...
            error(
                "CHROMOSOME $(useChr[c]): Wrong chromosome length(s) specified, please check your parameters!",
            )
        end
    ...
    println(
        "INFO: $numFragSelected out of $numFragTotal GBS fragments are selected after size-selection with lower and upper thresholds equals to [$lowerThresh, $upperThresh].",
    )
    ...
    println(
        "INFO: Expected sequencing coverage based on $numFragSelected selected GBS fragments is approximately $(round(mean(GBSFragCover)*100;digits=2))%.",
    )
    ...
    if plotOutput == true
        histogram(
            ...
            title = "Size Distribution of Raw GBS Fragments",
            label = "",
            fmt = :png,
        )
        savefig("RawFragSize")
        histogram(
            fragGBS[:, 4],
            normalize = :probability,
            title = "Size Distribution of GBS Fragments (after Size-Selection)",
            label = "",
            fmt = :png,
        )
        savefig("GBSFragSize")
        histogram(
            GBSFragCover,
            normalize = :probability,
            title = "Coverage of GBS Fragments per Mb",
            label = "",
            fmt = :png,
        )
        savefig("GBSCoverage")
